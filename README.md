...
- **Video loading** via WebCodecs + mp4box.js, frame-accurate, from a local folder
...
- **Intrinsics** per camera in a calibration worker with progress, optional
  coverage-based frame subsampling (default 80 frames/camera chosen for image
...
- **Bundle adjustment** with `@talmolab/sba-solver-wasm` in the worker: robust
  loss, outlier threshold, choose what to optimize, point cap, convergence chart,
  one-click revert.
...
```bash
node tests/run-mjs-tests.mjs           # pure-logic unit tests (no browser needed)
```
